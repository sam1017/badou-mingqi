...
@Author  : Song Jiazhen
@Date    : 2022/3/11
@Describe: 基于pytorch完成RNN分类任务网络编写
           RNN为单层，hidden_size为32,
           对于每个batch的RNN输出，可有不同方式得到RNN的最终输出：
           （1）以每个batch中最后一个隐含层结果作为最终输出
           （2）按照hidden_size方向进行最大池化，得到每个batch中所有样本在每个hidden_node下的最大值
           （3）按照hidden_size方向进行平均池化，得到每个batch中所有样本在每个hidden_node下的平均结果
           本程序选择第三种方法，对平均结果进行进行线性映射，得到概率
==========================================================================================="""
