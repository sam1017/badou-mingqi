...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        self.classify2 = nn.Conv2d(1,vector_dim, 128, 3)  # RNN循环层
...
def build_sample(vocab, sentence_length):
    ...
    # 指定哪些字出现时为正样本
    if set("abc") & set(x) and set('txl') & set(x):
        y = 3
    elif set("abc") & set(x) and not set('txl') & set(x):
        y = 2
    elif not set("abc") & set(x) and set('txl') & set(x):
        y = 1
    else:
        y = 0
...
def evaluate(model, vocab, sample_length):
    ...
    y = y.squeeze()
    print("本次预测集中共有%d个A样本, %d个B样本,%d个C样本,%d个D样本" % (
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        print("输入：%s, 预测类别：%d, 概率值：%f" % (input_string, int(torch.argmax(result[i])), max(result[i])))
