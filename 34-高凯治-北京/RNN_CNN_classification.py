...
def build_sample(vocab, min_sentence_length, max_sentence_length):
    ...
    if (set("ab") & set(x)) and not (set("cd") & set(x)):
        y = 1
    elif not(set("ab") & set(x)):
        y = 2
    # 指定字都未出现，则为负样本
    else:
        y = 0
...
class my_model(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab, hidden_size):
        ...
        self.classify = nn.Linear(vector_dim+3, 3)     #线性层
    ...
    def forward(self, x, y=None):
        ...
        x = self.drop(x)                           ##(batch_size, 67) -> (batch_size, 3)
        x = self.classify(x)                       #(batch_size, 67) -> (batch_size, 3)
...
def evaluate(model, vocab, min_sentence_length, max_sentence_length, batch_size):
    model.eval()
    x, y = build_dataset(batch_size*2, vocab, min_sentence_length, max_sentence_length)  #建立256个用于测试的样本
    x = padding_dataset(vocab, x)
    correct, wrong, sample0, sample1, sample2 = 0, 0, 0, 0, 0
        ...
        with torch.no_grad():
            ...
            for y_p, y_t in zip(y_pred, label):  #与真实标签进行对比
                if y_p == y_t:
                    ...
                    wrong += 1
        sample0 = sample0 + sum(label == 0)
        sample1 = sample1 + sum(label == 1)
        sample2 = sample2 + sum(label == 2)
    print("本次预测集中共有%d个标签为0的样本，%d个标签为1的样本, %d个标签为2的样本" % (sample0, sample1, sample2))
...
def main():
    ...
    train_sample = 2560    #每轮训练总共训练的样本总数
    char_dim = 64         #每个字的维度
...
def predict(model_path, vocab_path, input_strings, hidden_size):
    ...
    for i, input_string in enumerate(input_strings):
        print("输入：%s, 预测类别：%d" % (input_string, np.argmax(result[i].cpu().numpy()))) #打印结果
        print("预测结果")
        print(result[i])
...
if __name__ == "__main__":
    main()
    # test_strings = ["ab", "sgdf", "r", "cdkdwwjdhfjbakjfsjkagfsab"]
